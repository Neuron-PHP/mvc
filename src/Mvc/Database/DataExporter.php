...
class DataExporter
{
	...
	public function __construct(
		Config $PhinxConfig, string $Environment, string $MigrationTable = 'phinx_log', array $Options = [], ?IFileSystem $fs = null
	)
	{
...
'compress' => false
		...
		$this->_Adapter = AdapterFactory::instance()
	...
	private function shouldUseStreaming(): bool
	{
		...
		if( !in_array( $this->_Options[ 'format' ], [
			self::FORMAT_SQL,
			self::FORMAT_CSV
		] ) )
		{
	...
	private function getTableRowCount( string $table ): int
	{
		...
		if( $whereClause !== '' )
		{
			...
			{
				throw new \InvalidArgumentException( "Potentially dangerous WHERE clause detected for table '{$table}'. " . "WHERE clauses must not contain SQL commands, comments, or subqueries." );
			}
	...
	private function parseSimpleWhereClause( string $whereClause ): array
	{
		...
		if( empty( $parts ) )
		{
			throw new \InvalidArgumentException( "Cannot parse WHERE clause for parameterization: {$whereClause}. " . "Consider using the ORM QueryBuilder for complex queries." );
		}
		...
		for( $i = 0; $i < count( $parts ); $i++ )
		{
			...
			// Check for IN / NOT IN patterns first (these use parentheses)
			// Pattern allows empty parentheses to detect and error on them
			$inPattern = '/(\w+)\s+(NOT\s+IN|IN)\s*\(\s*([^)]*)\s*\)/i';
			if( preg_match( $inPattern, $part, $match ) )
			{
				$column    = $match[ 1 ];
				...
				// Values can be quoted ('value') or unquoted (123)
				// Pattern matches: quoted strings OR unquoted values (but NOT commas or parens)
				$values       = [];
				$valuePattern = '/\'((?:\'\'|[^\'])*)\'|"((?:""|[^"])*)"|([^,\s()]+)/';

				if( preg_match_all( $valuePattern, $valueList, $valueMatches, PREG_SET_ORDER ) )
				{
				...
				if( empty( $values ) )
				{
					throw new \InvalidArgumentException( "Empty IN list for column {$column}: {$part}" );
				}

				// Create placeholders for each value
				$placeholders         = implode( ', ', array_fill( 0, count( $values ), '?' ) );
				$quotedColumn         = $this->quoteIdentifier( $column );
				$parameterizedParts[] = "{$quotedColumn} {$operator} ({$placeholders})";
			...
			// (but IN/NOT IN are already handled above)
			if( strpos( $part, '(' ) !== false || strpos( $part, ')' ) !== false )
			{
				throw new \InvalidArgumentException( "Parentheses are only supported for IN/NOT IN operators: {$part}. " . "Consider using the ORM QueryBuilder for complex queries." );
			}

			// Check for IS NULL / IS NOT NULL patterns (these don't take a value)
			...
			else
			{
				...
				throw new \InvalidArgumentException( "Cannot parse WHERE condition: {$part}. " . "Consider using the ORM QueryBuilder for complex queries." );
			}
		}
	...
	private function escapeString( string $value ): string
	{
		...
		throw new \RuntimeException( "Cannot safely escape SQL values without PDO connection. " . "This adapter does not support safe string escaping. " . "Consider using prepared statements or upgrading to an adapter with PDO support." );
	}
